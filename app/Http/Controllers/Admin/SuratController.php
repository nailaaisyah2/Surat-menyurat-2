...
class SuratController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $tab = $request->input('tab') ?? session('admin_letters_tab', 'masuk');
        session(['admin_letters_tab' => $tab]);
        ...
        // Admin melihat surat masuk ke divisinya (bukan yang dikirim oleh divisinya sendiri)
        // Surat masuk = surat yang ditujukan ke divisi admin DAN pengirim bukan dari divisi admin
        $suratMasukQuery = Letter::where('penerima_division_id', $divisionId)
            ->whereHas('pengirim', function($query) use ($divisionId) {
                // Ambil surat yang pengirimnya bukan dari divisi admin
        ...
        // Admin melihat surat keluar dari divisinya (yang dikirim oleh user dari divisinya)
        // Surat keluar = surat yang dikirim oleh user dari divisi admin (tidak peduli ditujukan ke mana)
        ...
        return view('admin.letters.index', compact('letters', 'suratMasuk', 'suratKeluar', 'tab'));
    }
    ...
    public function setTab(Request $request)
    {
        ...
        session(['admin_letters_tab' => $request->tab]);
    ...
    {
        // Admin bisa membuat surat untuk divisi lain (tidak termasuk divisinya sendiri)
        $user = auth()->user();
        ...
        $divisions = $query->get();
        return view('admin.letters.create', compact('divisions'));
    }
    ...
    public function store(Request $request)
    {
        ...
        session(['admin_letters_tab' => 'keluar']);
    ...
    public function show(Letter $letter)
    {
        ...
        // Admin bisa melihat surat yang masuk ke divisinya atau surat keluar dari divisinya
        if (!$divisionId) {
            abort(403, 'Anda tidak memiliki akses untuk melihat surat ini.');
        }

        // Cek apakah surat masuk ke divisi admin atau surat keluar dari divisi admin
    ...
    public function respond(Request $request, Letter $letter)
    {
        ...
        // Pastikan admin hanya bisa menanggapi surat yang masuk ke divisinya
    ...
    public function downloadLampiran(Letter $letter)
    {
        ...
        // Admin bisa download surat yang masuk ke divisinya ATAU surat yang dia kirim
